/**
 * ContentNavigator Jquery Plugin https://www.jqueryscript.net/menu/Configurable-TOC-Page-Index-Generator-With-jQuery-contentNavigator.html
 * Licence: MIT
 * Copyright (c) 2016 aewebsolutions
...
function ContentNavigator(el, options){
    ...
    var init = function(){
        ...
        }*/ //Commented out by ParsedClassics
    ...
    var updateURL = function(block){
        if(block.node[0].id){
            ...
            //console.log("block.node[0].id: " + block.node[0].id);
            var href = window.location.href.replace(window.location.hash, "");
            window.history.replaceState("", document.title, href+'#'+block.node[0].id);
        }else if(block.parent){
            //console.log("block.parent.node[0].id: " + block.parent.node[0].id);
    ...
    this.goTo = function(block){
        ...
        // scroll center pane section into view  --- edited by ParsedClassics
        var resourcesPane = $("#" + ParsedClassicsVars.centerPaneContentId);  // edited by ParsedClassics
        resourcesPane.scrollTo($(block.node[0]), ParsedClassicsVars.animationSpeed); // edited by ParsedClassics
        // push info about selected section into JSON in URL --- edited by ParsedClassics
        var section_id = $(block.node[0]).find(":first-child").attr("id"); // edited by ParsedClassics
        if (section_id) {                                                       // edited by ParsedClassics
          var hash_string = ParsedClassicsVars.hashStringStartDocs + '{"section":"' + section_id + '"}'; // edited by ParsedClassics
          var location_string = window.location.href;                            // edited by ParsedClassics
          if (!window.location.hash) {                                           // edited by ParsedClassics
            location_string = location_string.replace("#", "") + hash_string;    // edited by ParsedClassics
          }                                                                      // edited by ParsedClassics
          else {                                                                 // edited by ParsedClassics
            location_string = window.location.href.replace(window.location.hash, hash_string); // edited by ParsedClassics
          }                                                                        // edited by ParsedClassics
          window.location.href = location_string;                                  // edited by ParsedClassics
        }                                                                         // edited by ParsedClassics
        self.desactivate();
        self.activate(block);
    };
