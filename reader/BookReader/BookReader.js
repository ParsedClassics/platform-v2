...
BookReader.prototype.initToolbar = function(mode, ui) {
    ...
    // <a class="BRTitleLink" href="javascript:void(0);" style="color: rgb(36, 94, 131); cursor: default; text-decoration: none;" title="The New Testament in the original Greek. Byzantine textform." target="_blank">
    
    var filename = String(window.location).split('?').shift().split('#').shift().split('/').pop().split('.').shift(); // by ParsedClassics: get file name in order to know is BookReder external or internal
    ...
    if (filename == "index") { // by ParsedClassics: Bookreader is external, i.e. not in <iframe/> element
      ...
      $('#BRreturn a')
        .addClass('BRTitleLink')
        .attr({'href': ".." + self.bookUrl.replace("embedded_bookreader", "index"), 'target': '_blank'}) // (1)".." added by ParsedClassics in order for the link to work correctly on localhost; (2) .replace("embedded_bookreader", "index") in order to open book outside <iframe/> in external reader; (3) added target="_blank"
        .html('<span class="BRreturnTitle">' + this.bookTitle + '</span>');
    }
...
BookReader.prototype.bindNavigationHandlers = function() {
    ...
    jIcons.filter('.onepg').bind('click', function(e) {
							...
							search_str = search_str.substr(1); // added by ParsedClassics
...
BookReader.prototype.showProgressPopup = function(msg) {
    ...
    //$(bar).css({
        //height:   '20px'
    //}).prop('className', 'BRprogressbar'); // changed by ParsedClassics
    $(bar).prop('className', 'loader8'); // added by ParsedClassics
